...
const Sidebar = (props: Props) => {
  ...
  return (
    <>
      <aside
        className={cn(
          "bg-primary-foreground dark:bg-primary-foreground z-[1149]",
          props.open ? "md:w-72 w-16" : "md:w-16 hidden md:block"
        )}
      >
        <nav className="nav">
          <ul
            className={cn(
              "flex flex-col gap-2",
              props.open && isMdScreen ? "px-4" : "px-2"
            )}
          >
            {items.map((item, index) => {
              return (
                <li key={index}>
                  <SidebarItem sidebarOpen={props.open} item={item} />
                </li>
              );
            })}
          </ul>
